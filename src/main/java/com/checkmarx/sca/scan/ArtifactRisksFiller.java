...
public class ArtifactRisksFiller {
    ...
    public boolean addArtifactRisks(@Nonnull RepoPath repoPath, @Nonnull ArrayList<RepoPath> nonVirtualRepoPaths) {
        ...
        if (nonVirtualRepoPaths.isEmpty()) {
            this._logger.warn(String.format("Artifact not found in any repository. Artifact name: %s.", repoPath.getName()));
            return false;
        } else if (this.scanIsNotNeeded(nonVirtualRepoPaths)) {
            this._logger.info(String.format("Scan ignored by cache configuration. Artifact name: %s", repoPath.getName()));
            return true;
        } else {
            ...
            try {
                ...
                if (this.FileShouldBeIgnored(repoPath, packageManager)) {
                    this._logger.debug(String.format("Not an artifact should be ignored. File Name: %s", repoPath.getName()));
                    return false;
                }
                ...
                this._logger.error(String.format("Exception Message: %s. Artifact Name: %s.", var9.getMessage(), repoPath.getName()), var9);
                return false;
            }
    ...
    public boolean addArtifactRisks(@Nonnull RepoPath repoPath, @Nonnull ArrayList<RepoPath> nonVirtualRepoPaths, boolean forceScan) {
        String repositoryKey = repoPath.getRepoKey();
        RepositoryConfiguration repoConfiguration = this._repositories.getRepositoryConfiguration(repositoryKey);
        if (nonVirtualRepoPaths.isEmpty()) {
            this._logger.warn(String.format("Artifact not found in any repository. Artifact name: %s.", repoPath.getName()));
        ...
        try {
            if (!this._repositories.exists(repoPath)) {
                ...
                if (properties != null && this.allPropertiesDefined(properties)) {
                    ...
                    this._logger.debug(String.format("There are missing properties, the scan will be performed. Artifact: %s", repoPath.getName()));
                    return false;
                }
            }
        } catch (Exception var6) {
            this._logger.error(String.format("Unexpected error when checking the last scan date for the artifact: %s", repoPath.getName()), var6);
            return false;
        }
    }

    private boolean allPropertiesDefined(Properties properties) {
        return properties.containsKey("CxSCA.TotalRisks") && properties.containsKey("CxSCA.LowSeverityRisks") && properties.containsKey("CxSCA.MediumSeverityRisks") && properties.containsKey("CxSCA.HighSeverityRisks") && properties.containsKey("CxSCA.RiskScore") && properties.containsKey("CxSCA.RiskLevel") && properties.containsKey("CxSCA.LastScanned");
    }
    ...
    private int getExpirationTime() {
        ...
        try {
            ...
            this._logger.warn(String.format("Error converting the 'sca.data.expiration-time' configuration value, we will use the default value. Exception Message: %s.", var3.getMessage()));
    ...
    private boolean FileShouldBeIgnored(RepoPath repoPath, IPackageManager packageManager) {
        ...
        boolean notCocoaPodsPackage = packageManager == PackageManager.COCOAPODS && !repoPath.getPath().endsWith(".tar.gz") && !repoPath.getPath().endsWith(".zip");
    ...
    private PackageAnalysisAggregation scanArtifact(@Nonnull ArtifactId artifactId) {
        ...
        try {
            ...
            this._logger.error(String.format("Failed to get artifact information. Exception Message: %s. Artifact Name: %s.", var5.getMessage(), artifactId.Name));
            return null;
        }

        try {
            return this._scaHttpClient.getRiskAggregationOfArtifact(artifactInfo.getPackageType(), artifactInfo.getName(), artifactInfo.getVersion());
        } catch (Exception var4) {
            this._logger.error(String.format("Failed to get risk aggregation of artifact. Exception Message: %s. Artifact Name: %s.", var4.getMessage(), artifactId.Name));
            return null;
        }
    }
    ...
    private void addArtifactAnalysisInfo(@Nonnull ArrayList<RepoPath> repoPaths, @Nonnull PackageAnalysisAggregation packageAnalysisAggregation) {
        ...
        for (RepoPath repoPath : repoPaths) {
            try {
                ...
                this._logger.error(String.format("Failed to add risks information to the properties. Exception Message: %s. Artifact Name: %s.", var6.getMessage(), repoPath.getName()));
            }
        }
    ...
    private void addArtifactAnalysisInfo(RepoPath repoPath, PackageAnalysisAggregation packageAnalysisAggregation) {
        VulnerabilitiesAggregation vulnerabilitiesAggregation = packageAnalysisAggregation.getVulnerabilitiesAggregation();
        List<String> licenceTypes = packageAnalysisAggregation.getLicenses();
        if (licenceTypes == null) {
            licenceTypes = List.of();
        }

        this._repositories.setProperty(repoPath, "CxSCA.TotalRisks", new String[]{String.valueOf(vulnerabilitiesAggregation.getVulnerabilitiesCount())});
        this._repositories.setProperty(repoPath, "CxSCA.LowSeverityRisks", new String[]{String.valueOf(vulnerabilitiesAggregation.getLowRiskCount())});
        this._repositories.setProperty(repoPath, "CxSCA.MediumSeverityRisks", new String[]{String.valueOf(vulnerabilitiesAggregation.getMediumRiskCount())});
        this._repositories.setProperty(repoPath, "CxSCA.HighSeverityRisks", new String[]{String.valueOf(vulnerabilitiesAggregation.getHighRiskCount())});
        this._repositories.setProperty(repoPath, "CxSCA.RiskScore", new String[]{String.valueOf(vulnerabilitiesAggregation.getMaxRiskScore())});
        this._repositories.setProperty(repoPath, "CxSCA.RiskLevel", new String[]{vulnerabilitiesAggregation.getMaxRiskSeverity()});
        this._repositories.setProperty(repoPath, "CxSCA.LastScanned", new String[]{Instant.now().toString()});
        this._repositories.setProperty(repoPath, "CxSCA.Licenses", new String[]{String.join(",", licenceTypes)});
    }
}
